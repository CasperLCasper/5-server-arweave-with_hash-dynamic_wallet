...
const App = Object.assign({}, AppState, {
  ...
  resetApp() {
    ...
    if (UI.generateNFTBtn) {
      ...
      UI.generateNFTBtn.dataset.price = ''; // SonarCloud Fix
    }
  ...
  // ==========================================
  // REFACTORED: generateNFT un palīgfunkcijas
  // ==========================================
  async generateNFT() {
    ...
    try {
      // 0. ANTI-BOT
      await this._verifyAntiBot();
      
      // 1. UZŅEM ATTĒLU UN VIDEO
      const media = await this._captureMediaFiles();
      
      // 2. APRĒĶINA PAGAIDU CONTENT HASH
      const tempContentHash = this._calculateTempContentHash(media.imageHash, media.videoHash);
      
      // 3. PĀRSLĒDZAS UZ BASE SEPOLIA UN AUTORIZĒJAS
      await this._prepareMintNetwork();
      
      // 4. requestMint — LIETOTĀJS MAKSĀ DEPOZĪTU
      const mintTx = await this._requestMintDeposit(media.imageHash, media.videoHash, tempContentHash);
      
      // 5. & 6. ARWEAVE AUGŠUPIELĀDE UN finalizeMint
      const finalResult = await this._processStorageAndFinalize(media, tempContentHash, snapshotData);
      
      // 7. & 8. ZIP FAILU SAGLABĀŠANA UN ALERTS
      await this._saveAndAlertSuccess(media, finalResult, mintTx);
      ...
      this._handleMintingError(error, previousShowInfo);
    } finally { 
  ...
  async _verifyAntiBot() {
    ...
    try {
      ...
      throw new Error(signError.message?.includes('User denied') || signError.code === 'ACTION_REJECTED' 
        ? 'USER_CANCELLED' 
        : 'Verification failed');
    }
  },

  async _captureMediaFiles() {
  ...
  _calculateTempContentHash(imageHash, videoHash) {
  ...
  async _prepareMintNetwork() {
    ...
    if (contractAddress) {
      try {
        ...
        UI.generateNFTBtn.dataset.price = `${mintPriceEth}`; // SonarCloud Fix
      } catch(e) {
  ...
  async _requestMintDeposit(imageHash, videoHash, tempContentHash) {
    ...
    return { hash: tx.hash, value: txValue };
  },

  async _processStorageAndFinalize(media, tempContentHash, snapshotData) {
    showToast('📤 Uploading to Arweave...', 'info');
    
    const nftFormData = new FormData();
    nftFormData.append('image', media.imageFile);
    nftFormData.append('video', media.videoFile);
    ...
    if (!serverRes.ok) throw new Error('UPLOAD_REFUND');
    ...
    const imageUrl = serverData.image.id ? `${gw}${serverData.image.id}` : `local://${media.imageHash}`;
    ...
    const localMetadata = {
      ...
      animation_url: media.videoFileName,
      attributes: [
    ...
    if (!media.videoBlob) delete localMetadata.animation_url;
    ...
    return { metaId, serverData, finalContentHash, localMetadataString, storageCostEth };
  },

  async _saveAndAlertSuccess(media, finalResult, mintTx) {
    const metadataBlob = new Blob([finalResult.localMetadataString], { type: 'application/json' });
    const metadataFileName = `metadata_${Date.now()}.json`;
    
    showToast('💾 Saving all files as ZIP...', 'info');
    const completeFiles = [
      { blob: media.imageBlob, filename: media.imageFileName },
      { blob: metadataBlob, filename: metadataFileName },
      { blob: media.videoBlob, filename: media.videoFileName }
    ];
    ...
    const arweaveSuccess = finalResult.serverData.arweave?.success || false;
    const arweaveStatus = arweaveSuccess ? '✅' : '⚠️';
    
    alert(`✅ NFT minted!\n\n` +
      `Tx: ${mintTx.hash}\n` +
      `Price: ${ethers.formatEther(mintTx.value)} ETH\n` +
      `(Storage: ${finalResult.storageCostEth} ETH)\n\n` +
      `🔐 Image Hash: ${media.imageHash}\n` +
      `🔐 Video Hash: ${media.videoHash}\n` +
      `🔐 Content Hash (Basescan & ZIP): ${finalResult.finalContentHash}\n` +
      `${finalResult.metaId ? '📄 Arweave Metadata: ' + finalResult.metaId + '\n' : ''}` +
      `${finalResult.serverData.image?.id ? '🖼️ Arweave Image: ' + finalResult.serverData.image.id + '\n' : ''}` +
      `${finalResult.serverData.video?.id ? '🎬 Arweave Video: ' + finalResult.serverData.video.id + '\n' : ''}` +
      `\n${arweaveStatus} Arweave: ${arweaveSuccess ? 'OK' : 'Failed (files saved locally)'}` +
      `\n\n💾 All files saved as nft_assets_*.zip`);
  },

  async _handleMintingError(error, previousShowInfo) {
    console.error(error);
    let userMessage = 'Something went wrong. Please try again.';
    
    if (error.message === 'USER_CANCELLED' || error.message?.includes('User denied') || error.code === 'ACTION_REJECTED') {
    ...
    } else if (error.message === 'UPLOAD_REFUND' || error.message?.includes('deposit has been refunded') || error.message?.includes('refunded automatically')) {
  ...
  async renderSnapshot(chain) {
    ...
    if (UI.generateNFTBtn) {
      ...
      UI.generateNFTBtn.dataset.price = price; // SonarCloud Fix
    }
  },
  ...
  init() {
    ...
    document.querySelectorAll('.theme-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        this.setAddonStyle(btn.dataset.theme); // SonarCloud Fix
      });
    });
