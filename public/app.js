...
const App = Object.assign({}, AppState, {
  ...
  async generateNFT() {
    ...
    // 🔥 SAGLABĀ DATUS PIRMS MINTĒŠANAS
    const savedTokens = [...this.tokens];
    const savedEthBalance = this.ethBalance;
    const savedTxCount = this.txCount;
    
    const previousShowInfo = this.showInfo;
    this.showInfo = false;
    
    try {
      const result = await this._executeNFTMinting(savedTokens, savedEthBalance, savedTxCount);
  ...
  async _executeNFTMinting(savedTokens, savedEthBalance, savedTxCount) {
    const snapshotEthBalance = savedEthBalance ? savedEthBalance.toString() : "0";
    const snapshotTxCount = savedTxCount ? savedTxCount.toString() : "0";
    const tokenList = savedTokens.filter(t => !t.isNFT).map(t => ({
      symbol: t.symbol, address: t.address, balance: t.balance
    }));
    const nftList = savedTokens.filter(t => t.isNFT).map(n => ({
      symbol: n.symbol, address: n.address, tokenId: n.tokenId
    }));
    const snapshotTokenCount = tokenList.length.toString();
    const snapshotNftCount = nftList.length.toString();
